...
def url(site):
    try:
        if OKadminFinder.checkUrl(site, proxies):
            ...
            urls = tqdm(OKadminFinder.getUrls('LinkFile/adminpanellinks.txt'),
        ...
        for url in urls:
            ...
            if OKadminFinder.checkUrl(reqLink, proxies):
                ...
                messenger.writeMessage('\n' + Fore.CYAN + Style.BRIGHT + '    {:<50}'.format(
                    '[✔] http://' + reqLink, ) + Fore.GREEN + Style.BRIGHT + '{:>30}'.format('Admin page found!\n'),
                                       'bright')
...
def random_proxy():
    proxy_list = requests.get(
        'https://raw.githubusercontent.com/a2u/free-proxy-list/master/free-proxy-list.txt').text.splitlines()
...
def proxy():
    ...
    try:
        ...
        print(
            '            ╟──╸ ' + Fore.YELLOW + Style.BRIGHT + 'don\'t add' + Fore.YELLOW + Style.NORMAL + ' \'http://\' or \'https://\'')
...
def ipinf():
    ...
    ┆''');
    print(
        '    ├──► ' + Fore.BLUE + 'Country: ' + cc + Fore.YELLOW + '    ├───► ' + Fore.BLUE + 'IP: ' + ip + Fore.YELLOW + '    ├────► ' + Fore.BLUE + 'Country ISO: ' + iso + Fore.YELLOW + '    └────► ' + Fore.BLUE + 'City: ' + city)
    print('')
...
def hos():
    ...
    print(Fore.CYAN + Style.BRIGHT + '\tServer: ' + Fore.YELLOW + rh.headers[
        'Server'] + '\t\t' + Fore.CYAN + Style.BRIGHT + 'Hostname: ' + Fore.YELLOW + di + '\n')
    try:
        xf = dict(rh.headers).get("x-frame-options")
        xf = str(xf)
        print(Fore.CYAN + Style.BRIGHT + '\tX-Powered-By: ' + Fore.YELLOW + rh.headers[
            'X-Powered-By'] + '\t\t' + Fore.CYAN + Style.BRIGHT + 'X-Frame-Options: ' + Fore.YELLOW + xf + '\n\n')
    except KeyError:
        pass
...
def interactive():
    try:
        ...
        try:
            ...
            rua = random.choice(ua)
            headers = {'user-agent': rua}
            print(Fore.BLUE + '\tGetting random user-agent...',
